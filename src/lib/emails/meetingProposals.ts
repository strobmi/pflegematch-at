...
export async function sendProposalEmail({
  ...
  proposals,
  matchesUrl,
}: ProposalEmailParams): Promise<void> {
  ...
  const { error } = await resend.emails.send({
    ...
    to,
    subject: "Terminvorschläge für Ihr Pflegegespräch – Bitte wählen Sie einen Termin",
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #2D2D2D;">
        <div style="background: linear-gradient(135deg, #C06B4A, #A05438); padding: 32px; border-radius: 12px 12px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 22px;">Terminvorschläge für Ihr Pflegegespräch</h1>
        </div>
        <div style="background: #fff; padding: 32px; border: 1px solid #EAD9C8; border-top: none; border-radius: 0 0 12px 12px;">
          ...
          <p>Ihr Vermittler hat folgende <strong>Terminvorschläge</strong> für ein Kennenlerngespräch per Videotelefonie vorbereitet. Bitte wählen Sie einen passenden Termin aus:</p>
          ...
          <p style="color: #888; font-size: 14px;">
            Falls keiner der Termine passt, können Sie im Portal auch eigene Alternativtermine vorschlagen.
          </p>
  ...
  if (error) {
    throw new Error(`Resend error (meeting proposals): ${error.message}`);
  }
}
